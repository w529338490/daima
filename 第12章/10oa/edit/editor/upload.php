...
if($act=='upload'){
	if($type=='attach'){
		$fileType=array('rar','zip','htm','doc','swf','fla','xls','html','txt','jpg','gif','bmp','png','jpeg');//允许上传的文件类型
	}
	elseif($type=='img'){
  ...
  set_path("../../".$upfileDir);
	...
	if(!in_array(strtolower($fileExt),$fileType))
		die("<script>alert('不允许上传该类型的文件！$fileExt');window.parent.\$('divProcessing').style.display='none';history.back();</script>");
