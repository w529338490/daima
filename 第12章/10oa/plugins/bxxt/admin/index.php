...
if (isset($user_id)){
          ...
          if($db->num_rows($result)==1){$adminrepair=1;$nav_admin=" | <a href=\"?filename=admin\" >管理</a>";}
                    if ($group_id==1){
          	  $adminrepair=1; 
	        ...
	        $login="<table width=\"99%\" height=32 border=\"0\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" class=tableborder_2>
                  <form id=\"form1\" name=\"form1\" method=\"post\" action=\"?filename=login\">  
	                   <tr>
                       <td>
                          当前用户： [$real_name] <a href=\"?filename=index\" >首 页</a> | <a href=\"#\" onclick=popUp('repair','0')>申请报修</a>  | <a href=\"?filename=list\" >我的报修</a>  $nav_admin
                       </td>
                     </tr>  
                   </form>
                  </table>";
...
$users[$r[userid]]=array($r[userid],$r[username],$r[realname]);
}
...
while($r=$db->fetch_array($result)){
	          ...
	          if ($user_id==$users[$r[user]][0]) $editpop="onclick=popUp('editrepair','$r[id]')"; else $editpop="";
	         	$table_show.="
	         	              <tr id=list$r[id]>
           	               <td align=\"left\" valign=\"middle\" height=24 class=$class_style>$r[address]</td>
           	               <td align=\"center\" valign=\"middle\" class=$class_style>".$users[$r[user]][2]."</td>
