...
<?
	...
	if($status=="removePic")
    {
		...
		$sql="update  ".$TableEb_product." set 	picture='' where id=".$id;
		$query=$db->query($sql);
	}
	...
	if($status=="edit")
	{
		...
		$remark_count="0";//评论数
			...
			if(!empty($file_name))
	    	{
				$sql="update  ".$TableEb_product." set  keywords='".$keywords."',eb_product_category_id=".$eb_product_category_id.",	product_name='".$product_name."',	market_price=".$market_price.",	price=".$price.",quantity=".$quantity.",	is_verify=".$is_verify.",	is_recomend=".$is_recomend.",	featured=".$featured.",content='".$content."',	publish_people='".$publish_people."',	publish_time='".$publish_time."',	picture='".$picture."' where id=".$id;
				$query=$db->query($sql);
				if($query)
				{
			...
			{
				$sql="update  ".$TableEb_product." set  keywords='".$keywords."', eb_product_category_id=".$eb_product_category_id.",	product_name='".$product_name."',	market_price=".$market_price.",	price=".$price.",quantity=".$quantity.",	is_verify=".$is_verify.",	is_recomend=".$is_recomend.",	featured=".$featured.",content='".$content."',	publish_people='".$publish_people."',	publish_time='".$publish_time."' where id=".$id;
				$query=$db->query($sql);
			}
					...
					<table  class="addTable"  border='0'  cellpadding='0'  cellspacing='1'>
								...
								<tr  class='tr2' >
									...
									<select name="eb_product_category_id" id="eb_product_category_id">
										<option value=""></option>
										<?
											$pid=$row['eb_product_category_id'];
								...
								<tr  class='tr2' >
									<td >产品名称:</td>
									<td ><input name="product_name" type="text" id="product_name"  value="<?=$row['product_name']?>" ></td>
								</tr>
								<tr  style="display:none;"  class='tr2' >
									<td >市场价格:</td>
									<td ><input name="market_price" type="text" id="market_price"  value="<?=$row['market_price']?>" ></td>
								</tr>
								<tr  class='tr2' >
									<td >当前价格:</td>
								...
								<script language="javascript">
									window.onload = function()
									{
										...
										setup('provinceid','cityid');
									}
								</script>
								<tr  class='tr2' >
									<td >描述:</td>
									<td >
									<textarea name="content" cols="70" rows="10" id="content"><?=$row['content']?></textarea>									</td>
								</tr>
								<tr  class='tr2'>
									<td >关键字:</td>
									<td ><textarea name="keywords" cols="70" rows="4" id="keywords"><?=$row['keywords']?>
									</textarea></td>
								</tr>
								<tr  class='tr2' >
									<td >产品图片:</td>
								  ...
								  {
								  ?>
                                      <img width="100" height="80"  src="../../upload/eb_product/s<?=$row['picture']?>" border="0" >
                                      <input name="button" type="button" class="button" onClick="removePic()" value="Delete">
                                      <?
								 }
								 ?>
                                      <br>
              				...
              				<tr  class='tr2' >
                				        <td colspan="2" align="center">
							     <input type="button"  class='button' name="Button" value="Submit" onClick="Check()">
							    &nbsp;&nbsp;&nbsp;&nbsp;
      						    <input type="button"  class='button' name="Submit2" value="Back" onClick="back()">						        </td>
              				</tr>
           			  </table>
			  </td>
          		</tr>
