...
<?
	...
	if($status=="remove")
	{
		...
		$sql="select count(*) as num from ".$TableInfo_category."  where pid=".$id;
	    $row=$db->getRow($sql);
		$b=false;
		if(!empty($row[num]))
		{
			echo "<meta http-equiv='Content-Type' content='text/html; charset=utf-8'>";
			echo "<script language='javascript'>alert('存在子级,请先删除子级!');</script>";
			$b=true;
		}
	
		$sql="select count(*) as num from ".$TableInfos."  where infos_category_id=".$id;
	    $row=$db->getRow($sql);
		if(!empty($row[num]))
		{
			echo "<meta http-equiv='Content-Type' content='text/html; charset=utf-8'>";
			echo "<script language='javascript'>alert('存在信息内容,请先删除内容!');</script>";
			$b=true;
		}
		...
		if($b==false)
		{
			...
			$sql="delete from ".$TableInfo_category." where id=".$id;
			$query=$db->query($sql);
	...
	if($status=="show")
	{
	  ...
	  $sql="update  ".$TableInfo_category." set is_show=1  where id in (".$filter;
	  $query=$db->query($sql);
	}
	...
	if($status=="unshow")
	{
	  ...
	  $filter=$filter.$checkednums[count($checkednums)-2].")";
	  $sql="update  ".$TableInfo_category." set is_show=2   where id in (".$filter;
	  $query=$db->query($sql);
	}
	...
	{
		$id=$_POST['id'];
		$sql="update ".$TableInfo_category." set sortid=".$_POST['sortid_'.$id]."  where id=".$id;
	    $db->query($sql);
	}
       		  ...
       		  <td align="center" valign="top">
				  <table class="containContentsTable">
              			...
              			<tr>
               				 <td colspan="2">
								 <?
									$sql="select * from ".$TableInfo_category."  where pid=0 order by sortid " ;
									$query=$db->query($sql);
									$sql1="select  * from  ".$TableInfo_category." where pid<>0  order by sortid " ;
									$result=$db->getAll($sql1);
									
									echo "<table  class='contentsTable'  border='0'  cellpadding='0'  cellspacing='1' >";
									echo "<tr class='tr1' align=center ><td >编号</td><td >菜单名称</td><td width=10%>链接方式</td><td width=15%>排序号</td><td width=10%>发布</td><td width=15%>访问权限</td><td width=10%>&nbsp;</td></tr>";
...
<script language="JavaScript" type="text/JavaScript">
	...
	function edit(id){
		var form=form1;
		location="info_categoryedit.php?id="+id;
	}

	function look(id){
		var form=form1;
		location="info_categorylook.php?id="+id;
	}
